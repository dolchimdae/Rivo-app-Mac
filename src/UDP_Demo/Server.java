...
public class Server {
    ...
    public static boolean crc16_check(byte[] data){             //data == bufferToReceive
        ...
        System.out.println("computed crc: " + crc_i);
        System.out.println("received crc: " + realCrc_i);
            ...
            switch (ID) {
                ...
                case "UM":
                    if (opcode == 0x0) {          //File receive start
                        ...
                        while (true) {
                            dpReceive = new DatagramPacket(new byte[MAX_BUFFER_SIZE], MAX_BUFFER_SIZE);
                            ds.receive(dpReceive);
                            String receivedStr = new String(dpReceive.getData()).trim();

                            if (receivedStr.equals("start")) {
                                dpReceive = new DatagramPacket(new byte[MAX_BUFFER_SIZE], MAX_BUFFER_SIZE);
                                ds.receive(dpReceive);
                                fileName = new String(dpReceive.getData()).trim();

                                //받은 파일 저장위치 지정
                                fileToReceive = new File("C:\\Rivo_Storage\\" + fileName);
                                fos = new FileOutputStream(fileToReceive);
                            } else if (receivedStr.equals("end")) {
                                System.out.println("Receive Complete");
                                fos.close();
                                break;
                            } else {
                                int readBytes = dpReceive.getLength();
                                fos.write(dpReceive.getData(), 0, readBytes);
                            }
                        }
